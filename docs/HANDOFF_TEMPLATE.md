...
Default split: **chat** owns PR-open/merge actions and any git write that's PAT-gated (pushing branches, opening PRs, merging) at the point Hitesh is present to approve them in-conversation; **CC** owns the coding itself (branching, editing, committing, verifying locally, pushing its own feature branches). This split exists so it's always clear who does what without re-deriving it each session — but it is a default, not a law: if a session does work outside its normal lane (e.g. a chat session pushing/merging its own PRs directly, as happened across the `GEN-2609-073`/`-074` sessions), **log it explicitly as a one-off exception here**, with which sessions and why, rather than letting it quietly become an unstated new precedent. Per-item in sections 2/3, note which lane each open item belongs to.
