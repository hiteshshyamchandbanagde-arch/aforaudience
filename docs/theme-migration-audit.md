...
12. ~~Auth-page full white shell (`login`/`register`/etc.)~~ — **DONE 5 Sep 2026.** Got its own design pass (Figma Make, `docs/design.md` "Auth Pages — Dark Theme Redesign") rather than a token swap, as anticipated here. Shipped as PR #551 (Register) + #552 (Login/Forgot Password/Reset Password/Verify Email). One related gap found during that work, not yet fixed: Login's registered/password-updated success banners still hardcode a `#68D391` border instead of a token — see `docs/design.md`'s Auth Pages status update for detail.
