...
- "Wall of Fame" — Artist of the Month / Event of the Month, computed directly from existing `Review.rating` (already tied to Event + optional Performance). See §9.3 — schema-ready, no UI. **Next candidate for pickup, per 19 Jul session close.**
...
---
*Document version: 3.1 — Twentieth amendment (19 Jul 2026 session: 9 PRs merged to qa, #104–#112. Standing process change: sessions now start with a Feedback-table + design-doc reconciliation before treating any carried-over item as still open — this caught 3 false-urgent items in the first pass alone (marquee already fixed, venue-owner OTP already deliberately resolved by design, "paid event free" was actually just a cosmetic label). Real fixes shipped: header displayName staleness (#104, missing `useSession().update()` call), free-checkout ₹0→"Free" label (#104), username-suggestion off-by-one (#105), silent admin action failures on approvals + feedback status (#106, found while scoping the toast rollout, not from a report), full toast rollout across the 9 genuine candidate pages out of the original ~16 estimate (#107–#110, see item 10), review-widget client-side gating closing a gap flagged since the nineteenth amendment (#111), and a live crash on event creation — unbounded `maxPerformers` input caused a raw Postgres integer-overflow, found by correlating a same-day feedback report against Vercel's runtime error log rather than guessing at the reporter's own theory, which turned out to be wrong (#112). §9.3 candidates for the next session: Wall of Fame / Top Venues-Organisers leaderboard (schema-ready, next planned pickup), admin bookings list surfacing `Booking.deliveryError`, K4 admin-wide revenue rollup, H3 flag/suspend accounts, persona value-prop homepage blocks.)*
