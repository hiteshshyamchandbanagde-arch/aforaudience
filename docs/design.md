...
- **Seat-map builder for Venue Owners (BookMyShow-style numbered seating)** — Hitesh request (19 Jul). Current `Venue.seatMap` is section-level only (a count + price per section, e.g. "Front: 40 seats, ₹250"), not individual numbered seats. Confirmed with Hitesh: venues are a genuine mix of fixed-seating and general-admission, so this needs to be an opt-in second mode per venue, not a replacement. Proposed shape (not yet built): General Admission mode stays exactly as-is; new Numbered Seating mode gives the Venue Owner a builder to lay out rows/seats (with pricing-section assignment), producing a real seat grid, plus new `Seat`/`SeatHold` models and a seat-picker UI on the audience side (extending the existing 15-min PENDING-booking TTL pattern to per-seat holds). Flagged as a multi-session build needing its own dedicated design pass, not something to scope inline - not started.
