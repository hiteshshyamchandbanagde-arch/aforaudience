...
| **Pre-existing type error blocks `next build`'s typecheck step** | Found 19 Jul session 2, not touched by either PR that session (#113, #114). `src/app/api/auth/otp/request/route.ts:58` — `generateAndSendOtp(targetPhone, purpose)` where `targetPhone` is typed `string \| null`. Turbopack's compile step succeeds regardless (this only surfaces in the separate typecheck pass), so it hasn't blocked any deploy yet, but it means `next build`'s typecheck can't be trusted to catch a *new* error introduced elsewhere in the same build — every PR since has had to fall back to `tsc --noEmit -p tsconfig.json` and grep for just the touched files to get a real signal. Worth a real fix (narrow the type or guard the null) next session it's convenient. |
...
- **Top Venues / Top Organisers leaderboard.** No direct rating model exists for either — decided proxy: average rating of their events, minimum 3 reviews to qualify (same rule as above, for consistency). Natural fast-follow on the Wall of Fame pattern just shipped — same aggregation shape, different grouping key.
...
---
*Document version: 3.2 — Twenty-first amendment (19 Jul 2026, session 2: 2 PRs merged to qa, #113–#114, both Vercel-verified green on the exact merged commit sha before merge). **(1) Persona value-prop blocks (#113):** the homepage "who's it for" role directory was a plain icon/desc/tag grid — no actual pitch. Rebuilt as real value-prop blocks for Audience/Artist/Organiser/Venue Owner: one-line pitch + 3 concrete benefit bullets + a dedicated CTA into the real opt-in-upgrade flow (`/profile`, per §2.2). Admin dropped from this section — not a public conversion target. Copy + layout only. **(2) Wall of Fame (#114):** Artist of the Month / Event of the Month shipped exactly per the §9.3 decision already on record — min 3 reviews, calendar month scope, no schema changes, aggregated in JS since `artistId` is two joins from `Review` and can't be expressed as a single Prisma `groupBy`. New page `/wall-of-fame`, added to `SiteNav`. **(3) Two stale §9.3/handoff entries corrected, not from new work:** the admin `deliveryError` bookings page had actually shipped 15 Jul but was never marked done — caught before being built a second time; and the "next unblocked work" list's Wall of Fame entry is now reconciled against its actual §9.3 decision text rather than left as a duplicate open item. Standing-process note: §9.3 needs the same reconciliation discipline as the Feedback table at session start, not just a one-time read — this is the second session in a row where the process caught something stale, which is the point of having it. **(4) Found, not fixed:** a pre-existing type error in `api/auth/otp/request/route.ts` blocks `next build`'s typecheck step (unrelated to anything touched this session or last) — see §9.2 row. §9.3 candidates for next pickup: Top Venues/Organisers leaderboard (same aggregation pattern as Wall of Fame, different grouping key), K4 admin revenue rollup, H3 flag/suspend accounts.)*
