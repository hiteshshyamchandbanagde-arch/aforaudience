...
**Twelfth amendment — 17 Jul 2026 session, continued.** One more PR (#62) plus a full feedback-log pass. **(1) Two feedback-driven fixes shipped (#62):** password/confirm-password eye-toggle on the register form (`cmrlxz35q`), and a "Get Directions" Google Maps link on venue detail pages (`cmrly74zi`) — `Venue.lat`/`lng` had existed in the schema unused since early on. Both zero-decision, additive UI only. **(2) Full feedback triage completed:** the eleventh amendment's session had only reviewed 8 of 18 total `Feedback` rows; this pass reviewed the remaining 10, corrected one that was still `NEW` despite being fixed weeks ago (APK address bar, PR #50), and marked the two just-shipped items `RESOLVED`. All 18 rows now carry a real status — nothing left dangling. **(3) Two critical gaps surfaced that have no EPIC at all:** (a) **no ticket check-in/scan flow anywhere** — schema, API, and frontend all checked; the QR code on the ticket PDF encodes a raw booking ID but nothing validates it at the door, marks a booking used, or prevents duplicate entry. Arguably more operationally critical than E3/E5, since without it a real event can't actually be gated by ticket ownership. (b) **no real legal pages** — `/privacy` is a `ComingSoon` placeholder, `/terms` doesn't exist as a route. Both logged in §9.1 (now items #1–#2, reordered above the phone-verification gate) and §9.2. **(4) Feedback-status-tracking backlog item added (§9.4), explicitly low priority per Hitesh.** Scoped in the doc: status enum without version-number promises (no versioned release cadence exists to promise against), user-initiated close/reopen on resolved items, and a dependency on the notification-system gap (also newly logged in §9.4) since a status update nobody sees isn't useful. Sized as a real feature, not scheduled for immediate build.
...
1. **Ticket check-in/scan flow — no EPIC exists for this at all.** See §9.2. The QR code on the ticket PDF encodes the raw booking ID, but nothing anywhere (schema, API, UI) reads it back, marks a booking as used, or stops the same ticket being shown at two gates or reused for re-entry. More operationally critical than E3/E5 — without it, a real event can't actually be gated by ticket ownership.
2. **Legal pages — Privacy Policy is a placeholder, Terms of Service doesn't exist.** `/privacy` renders `ComingSoon`; no `/terms` route at all. Needed before company registration / real payments — Razorpay merchant compliance and basic exposure on collecting phone/email/payment data. Content-drafting + founder/legal sign-off, not a big engineering lift.
3. Gate ticket booking on phone-verified users only — see finding in §9.2. Needs a "verify your phone" completion flow first (no UI exists today outside the immediate post-registration OTP screen); reuse existing OTP send/verify endpoints rather than building new ones. Small-medium — real UI flow, not a one-file patch.
4. E3 — lineup drag-and-drop builder (larger, standalone)
5. E5 — real-time ticket sales dashboard (larger, standalone)
6. PWA screenshots in the manifest — needs 2-3 real screenshots of the app on a phone; ~15 min from Claude once the images exist
7. Prod Play Store package — repeat PWABuilder against `https://www.aforaudience.com` with package ID `com.aforaudience.app` (reserved for this) and a **permanent signing key** (never lose). Only when Razorpay live keys are in and real Play Store submission is desired (weeks out).
8. **(Low priority, explicitly deprioritized by Hitesh)** User-facing feedback status tracking — see §9.4 for scoping.
...
| Ticket booking has no phone-verification gate | Password login and both booking routes (`/api/bookings`, `/api/venue-bookings`) never check `User.isVerified`. An account can register, skip the phone-OTP step entirely, and book/pay for a real ticket with a phone nobody can reach — real money and event-day logistics (ticket delivery, no-show/dispute resolution) riding on an unverified contact. Deliberately not fixed same-session: doing it properly needs a "verify your phone" completion UI first (only entry point today is the immediate post-registration OTP screen), not just adding the check. See §9.1 #3. Email verification, by contrast, is already correctly non-blocking by design — confirmed via code review, no gap there (see `verify-email/route.ts` comment). |
| No ticket check-in/scan flow anywhere | Checked schema (`Booking` has no `checkedInAt`/`attended`/equivalent), API routes, and frontend — nothing validates a ticket's QR code at the door or marks it used. Not a partially-built feature, a total blind spot; no EPIC in §6 covers it. See §9.1 #1. |
| No real legal pages | `/privacy` is a `ComingSoon` placeholder; `/terms` doesn't exist as a route at all. See §9.1 #2. |
| Venue detail page never surfaced `Venue.lat`/`lng` | Schema had coordinates since early on, UI never used them — no map, no directions link. ✅ **Fixed this session (PR #62)** — "Get Directions" link added, precise pin when coordinates exist, address-search fallback otherwise. Originated from Feedback id `cmrly74zi`. |
...
- **A5 — Search.** No standalone search feature exists; only in-page filters on Events/Artists listings. Reinforced by live feedback (`cmrlz2r0m`, "Search Engine Suggestion") — a real felt gap, not just a checklist item.
...
- **General notification system.** EPIC J only covers SMS (blocked on MSG91 DLT). There is no in-app or email notification of any kind beyond transactional emails (ticket, password reset, email verification) — no "your event starts soon," no booking-status-change alert, nothing. Surfaced by feedback (`cmrlusv5k`, "Mobile notification") and is also what the feedback-status-tracking item below would need to actually notify a user of a status change.
- **User-facing feedback status tracking (low priority — Hitesh's explicit call, 17 Jul).** Idea: reporter can see their feedback's status (New/In Progress/Resolved/Won't Fix — no version numbers, since there's no versioned release cadence to promise against) and can close or reopen a `Resolved` item themselves, turning "resolved" from Claude/Hitesh's claim into something the reporter confirms. Needs: (a) a `userId`-scoped "My Feedback" view — `Feedback.userId` already exists; (b) a public-facing status label (distinct from the internal NEW/REVIEWED/RESOLVED admin enum, or a mapping over it); (c) `closedByUser` boolean or similar so a resolved item can be user-reopened; (d) a way to notify the user on status change — blocked on the notification-system gap immediately above, since a status that only updates if the user remembers to check back isn't much better than nothing. Not sized as a one-file patch; scope it properly when picked up.
...
---
*Document version: 2.8 — Twelfth amendment (17 Jul 2026: PR #62 shipped — password toggle + venue Maps link; full feedback-log triage completed (18/18 rows now have real status); two critical no-EPIC gaps surfaced and logged — ticket check-in/scan flow, legal pages; feedback status-tracking added to backlog as explicit low priority)*
