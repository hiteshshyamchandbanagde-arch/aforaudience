...
| Dashboard nav link | Role-based users can only reach Dashboard via Profile, not from the header nav. Small nav-only fix. **Live-confirmed again (21 Jul, session 12)** - Vijay (Venue Owner) had to go through Profile to reach "Your Venues"; no direct top-nav entry. Still not fixed. |
...
- ~~Follow Artist/Venue/Organiser for update notifications~~ ✅ **Shipped for Artist + Venue (session 14, 23 Jul), Organiser deferred.** Hitesh request (22 Jul, session 13); refined further same session (23 Jul) after Hitesh asked for a YouTube-style separate notification toggle rather than notifications always firing on follow. **Follow table migrated live on QA** from Artist-only (`artistId` direct FK) to polymorphic (`targetType` enum `ARTIST|VENUE|ORGANISER` + `targetId`, no FK - one table instead of three near-identical ones), plus a `notifyEnabled` boolean (defaults **true** - unlike YouTube, the entire stated reason to follow here is hearing about new events, so defaulting off would bury the feature behind a step almost nobody takes). UI: heart toggle + a bell icon (visible only once following, matching YouTube's own pattern) on the Artist and Venue detail pages. Push trigger: genuine `DRAFT`/`PENDING_APPROVAL` → `APPROVED` transition only (both at creation and on later publish via edit) - re-saving an already-live event does not re-notify. Shares delivery plumbing (`sendPushToUser`) with the pre-existing "new follower" push, which is untouched and coexists as a separate trigger. **Organiser follow not built:** no public Organiser profile page exists anywhere in the app yet (confirmed - `src/app/(public)` has no `organisers` route), so there's no page to put the toggle on. The API-layer helper (`src/lib/follow.ts`) is already generic across all three target types, so adding Organiser is just a route + UI page once that profile page exists - not re-architecting. Also fixed in the same pass: `Artist._count.followers` (relied on the now-removed direct relation) recomputed as a manual `Follow` count on the Artist profile server page.
- **Reply-to-comment** — Hitesh request (22 Jul, session 13). Once an audience member comments, the recipient should be able to reply. **Scoped (collaborator decision, not yet built):** v1 is owner-only reply (the Artist/Venue/Organiser being commented on can reply; not open threaded discussion among all audience members) — much simpler UX/moderation surface, ships faster. One level deep, no arbitrary nesting. Schema: self-referencing nullable `parentId` on the existing Comment table rather than a separate Reply model. Notify the original commenter on reply via the same `sendPushToUser` call used above.
