...
- ~~Netflix-style intro/splash screen~~ ✅ **Shipped (PR #122, 19 Jul), refined twice same day (PR #124, #126).** Text-based wordmark animation (matches the existing SiteNav "A"+"forAudience" mark), plays once per browser session via sessionStorage, mounted once in the root layout so it doesn't replay on internal route changes. Respects prefers-reduced-motion (skips entirely). **Refinement 1 (PR #124):** live PWA testing on Android caught that the OS-generated PWA splash (Android auto-builds this from the manifest icon before any JS runs) showed the app's three-bar icon mark, then the custom intro cut straight to the wordmark - two disconnected beats. Custom intro now leads with the same three bars from `src/app/icon.svg`, bottom-up reveal (white base, amber, then the orange cap). **Refinement 2 (PR #126):** further live testing found the icon-to-wordmark size jump still felt abrupt (OS splash icon is large, ours was 72px), and the wordmark's scale-in-as-one-block read as generic. Icon now sized closer to the OS splash scale (`clamp(120px, 34vw, 220px)`) and shrinks+fades into the wordmark's "A" rather than a flat crossfade at mismatched sizes. "forAudience" now cascades in letter by letter (35ms stagger) instead of scaling in as a block - chose this over a blinking-cursor typewriter effect since a cascade matches the warm editorial serif brand better than a terminal-style reveal. Total runtime ~2.95s. Also worth recording: a burst of 6 back-to-back deployments in ~30 minutes while Hitesh had the site/PWA open on his own devices caused a transient "stuck loading" / blank-page symptom (likely a live tab's in-flight JS chunk request landing on a build that had already rotated out) - not a code defect, resolved itself with a hard refresh / PWA reopen. Lesson: pace deploys with a beat in between when the person has the app open and is actively testing live, rather than shipping several in a tight window.
...
---
*Document version: 3.3 — Twenty-second amendment (19 Jul 2026, session 2 continued: PR #115 merged to qa, Vercel-verified green on the exact merged commit sha). Top Venues / Top Organisers leaderboard shipped — all-time ranking (deliberately not calendar-month scoped, unlike the two "of the Month" awards), same 3-review floor, top 5 each, added as two sections on the existing `/wall-of-fame` page rather than a new nav entry. Same PR also fixed the pre-existing `otp/request/route.ts` type error flagged in the twenty-first amendment — which promptly surfaced a second, different pre-existing type error hiding behind it (`api/events/[id]/attendees/route.ts`, implicit `any`), left unfixed and logged as its own §9.2 row since `next build`'s typecheck only ever shows the first error it hits, so there may be a queue of these rather than just two. Session 2 total: 3 PRs (#113–#115), all Vercel-verified before merge, docs kept in lockstep with each merge rather than batched at session end.)*
