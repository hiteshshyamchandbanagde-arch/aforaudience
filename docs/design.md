...
- ~~Netflix-style intro/splash screen~~ ✅ **Shipped (PR #122, 19 Jul).** Text-based wordmark animation (matches the existing SiteNav "A"+"forAudience" mark) - scale-in, shimmer sweep across the "A", brief hold, fade out (~1.75s total). Plays once per browser session via sessionStorage, mounted once in the root layout so it doesn't replay on internal route changes. Respects prefers-reduced-motion (skips entirely). No new image assets.
