...
10. ✅ **Toast notification system** — shipped (PR #98, 18 Jul). `ToastProvider`/`useToast()` in `src/components/Toast.tsx`, mounted globally. Pilot: venue create + event create forms via a `fail(message)` helper that sets the existing inline error banner *and* fires a toast. Same `setError(err.message)` pattern exists on ~16 other dashboard pages — not yet rolled out there, see below.
...
**Newly unblocked, not yet started (identified session 5, 18 Jul):**
- Roll the toast `fail()` pattern out to the ~16 remaining dashboard pages using the old `setError(err.message)`-only pattern (edit forms, sales pages, applications, venue-requests, etc). Mechanical, same shape each time, now that the pattern is confirmed working on venue/event create.
