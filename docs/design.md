...
| E5 | As an Organiser, I can view real-time ticket sales | 5 | ✅ Shipped (fifteenth amendment — per-event dashboard; sixteenth amendment — overview + range filters). Per-event: polling dashboard (20s), per-tier sold counts, revenue split, sales timeline, recent bookings. Overview (`/dashboard/organiser/sales`): cross-event aggregate with Week/Month/Quarter/Year/All Time range filter, per-event table drilling into the per-event page. Extended same session (fifteenth) to a Venue Owner revenue view — Venue Owner overview + organiser-breakdown deferred to next session. |
...
4. **Venue Owner sales overview** (next session) — same pattern as the Organiser overview: aggregate across all venues, range filter, per-venue drill-down (existing `/dashboard/venue/[id]/sales` page), plus an organiser-breakdown table (which organisers rent the most / spend the most). Decision going in: table on the overview, not a dedicated per-organiser page — revisit only if real usage shows a need.
...
---
*Document version: 3.0 — Sixteenth amendment (17 Jul 2026: E5 phase 2 — Organiser sales overview at `/dashboard/organiser/sales` with Week/Month/Quarter/Year/All Time range filters, cross-event revenue/tickets/bookings aggregate, per-event drill-down table. Per-event sales endpoint and page retrofitted with the same range param — seat-occupancy and pending reservations stay all-time/current-state, revenue/timeline/recent-bookings scope to the range. PR #75 merged to qa. Venue Owner overview + organiser-breakdown deferred to next session.)*
