...
- **Follow Artist/Venue/Organiser for update notifications** — Hitesh request (22 Jul, session 13). Audience can mark an Artist/Venue/Organiser as favorite and get notified on new activity from them. **Scoped (collaborator decision, not yet built):** v1 trigger is new published event/gig only, not profile edits — keeps scope tight and avoids notification noise. Single polymorphic `Follow` table (`followerId, targetType` enum `ARTIST|VENUE|ORGANISER`, `targetId`, unique on the triple) rather than three near-identical per-type tables. UI: heart/star toggle on the public profile page. Notification channel: push only, reusing the existing `sendPushToUser` (EPIC J plumbing) — deliberately not forcing an in-app notification center into scope just for this; that's a bigger surface to build separately if ever needed. "Never tax the scene" check: pure audience-side engagement, no money involved, no conflict with the core principle. Shares delivery plumbing with the feedback-loop notification feature (also scoped, not yet built) — good candidate to build alongside it.
