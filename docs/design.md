...
- ~~Netflix-style intro/splash screen~~ ✅ **Shipped (PR #122, 19 Jul), refined same day (PR #124).** Text-based wordmark animation (matches the existing SiteNav "A"+"forAudience" mark) - scale-in, shimmer sweep across the "A", brief hold, fade out. Plays once per browser session via sessionStorage, mounted once in the root layout so it doesn't replay on internal route changes. Respects prefers-reduced-motion (skips entirely). **Refinement (PR #124):** live PWA testing on Android caught that the OS-generated PWA splash (Android auto-builds this from the manifest icon before any JS runs) showed the app's three-bar icon mark, then the custom intro cut straight to the wordmark - two disconnected beats. Now the custom intro leads with the same three bars from `src/app/icon.svg`, bottom-up reveal (white base, amber, then the orange cap, per Hitesh's suggestion - reads as building the mark up rather than falling), settles, then crossfades into the wordmark. Total runtime ~2.7s (was ~1.75s) - still snappy, just accounts for the extra beat.
