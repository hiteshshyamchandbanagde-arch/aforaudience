...
| **No way back to Guided-Setup-vs-Draw-It-Myself once seats exist ("no back navigation after save")** | Hitesh live report (21 Jul), confirmed via code review, **still not fixed - correction to this entry, session 12**. `seat-map/page.tsx` has two different back-links, and mid-session live testing initially conflated them: (1) `"← Back to venue"` (line ~675, unconditional, exits the builder entirely back to the venue detail page) and (2) `"← Back to setup options"` (line ~1042, calls `backToChoice()` to switch Guided Setup ↔ Draw It Myself) - **only the second one is wrapped in `{seats.length === 0 && (...)}`, and that guard is still in the code, unfixed.** Live-testing this session found link (1) present but easy to miss below a long seat layout (300+ seats) - that's a real but different, minor scroll/placement issue, not this bug. Link (2) - the actual setup-choice escape hatch - genuinely still disappears once any seat exists, exactly as originally diagnosed. `backToChoice()` only flips local view state (`builderPath`/`wizardStep`) and never touches `seats`, so the guard isn't protecting any real data loss. **Suggested fix (small, safe, still open):** drop the `seats.length === 0 &&` guard on the "← Back to setup options" link so it always shows, matching the wizard's own unconditional "Change approach" equivalent. |
