...
| **No way back to Guided-Setup-vs-Draw-It-Myself once seats exist ("no back navigation after save")** | Hitesh live report (21 Jul), confirmed via code review, not yet fixed. `seat-map/page.tsx`: the canvas view's "Back to setup options" link (calls `backToChoice()`, which only flips local view state - `builderPath`/`wizardStep` - and never touches the `seats` array, so it's non-destructive) is wrapped in `{seats.length === 0 && (...)}` - it only renders when the canvas is still empty. The wizard's own equivalent link ("Change approach") has no such guard and is always shown mid-wizard. The effect: the moment Guided Setup finishes (which by definition produces seats > 0) or any seat is placed in Draw It Myself, this escape hatch vanishes permanently - there's no way back to the choice screen from inside the builder, only the page-level "Back to venue" link at the very top, which exits the builder entirely rather than letting you switch approach. Likely an accidental "hasn't started yet" guard that doesn't match what the linked action actually does. **Suggested fix (small, safe):** drop the `seats.length === 0 &&` guard so the link always shows, matching the wizard's own unconditional version - since `backToChoice()` never clears `seats`, there's no data-loss risk in showing it unconditionally. |
