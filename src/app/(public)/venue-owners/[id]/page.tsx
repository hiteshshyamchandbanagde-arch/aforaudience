    ...
    <main style={{ minHeight: "100vh", background: "var(--afa-surface-page)", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ background: "var(--afa-surface-raised)", padding: "48px 24px", borderBottom: "1px solid rgba(245,245,240,0.1)" }}>
        <div style={{ maxWidth: "800px", margin: "0 auto", display: "flex", gap: "24px", alignItems: "center", flexWrap: "wrap" }}>
          <div style={{ width: "96px", height: "96px", borderRadius: "50%", background: "rgba(245,245,240,0.1)", border: "3px solid rgba(245,245,240,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "36px", fontWeight: 700, color: "var(--afa-text-primary)", flexShrink: 0, overflow: "hidden" }}>
            {owner.user.avatar ? (
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))", gap: "16px" }}>
            {owner.venues.map((v) => (
              <Link
                key={v.id}
                href={`/venues/${v.id}`}
                style={{ display: "block", background: "var(--afa-surface-raised)", borderRadius: "3px", overflow: "hidden", border: "1px solid rgba(245,245,240,0.1)", textDecoration: "none" }}
              >
                <div style={{ height: "120px", background: "var(--afa-surface-inverse)" }}>
                  {v.photos?.[0] && (
                    // eslint-disable-next-line @next/next/no-img-element
