  ...
  return (
    <main style={{ minHeight: "100vh", background: "var(--afa-surface-raised)", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ maxWidth: "760px", margin: "0 auto", padding: "48px 24px" }}>
        ...
        {organiser.code && (
          <p style={{ fontSize: "13px", color: "var(--afa-text-primary)", opacity: 0.4, marginBottom: "8px" }}>{organiser.code}</p>
        )}

        <OrganiserFollowButton organiserId={organiser.id} />

        {organiser.bio && (
          <p style={{ fontSize: "14px", color: "var(--afa-text-primary)", opacity: 0.75, lineHeight: 1.6, marginBottom: "28px", maxWidth: "600px" }}>
            {organiser.bio}
          </p>
        )}
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                    {upcoming.map((e) => (
                      <Link
                        key={e.id}
                        href={`/events/${e.id}`}
                        style={{ display: "block", padding: "12px 16px", background: "var(--afa-surface-raised)", borderRadius: "8px", textDecoration: "none", color: "var(--afa-text-primary)" }}
                      >
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
                    {past.slice(0, 10).map((e) => (
                      <Link
                        key={e.id}
                        href={`/events/${e.id}`}
                        style={{ display: "block", padding: "12px 16px", background: "var(--afa-surface-raised)", borderRadius: "8px", textDecoration: "none", color: "var(--afa-text-primary)", opacity: 0.7 }}
                      >
                        <div style={{ fontSize: "14px", fontWeight: 600 }}>{e.title}</div>
                        <div style={{ fontSize: "12px", opacity: 0.6 }}>
                          {new Date(e.date).toLocaleDateString("en-IN", { day: "numeric", month: "short", year: "numeric" })}
