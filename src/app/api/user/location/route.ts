...
export async function GET() {
  ...
  if (resolved.source === 'detected' && resolved.city) {
    res.cookies.set(LOCATION_COOKIE, locationCookieValue(resolved.city, resolved.lat, resolved.lng), LOCATION_COOKIE_OPTIONS)
  }
  return res
}
...
export async function POST(req: Request) {
  ...
  if (session?.user) {
    await prisma.user.update({
      where: { id: (session.user as any).id },
      data: { defaultCity: cleanCity, defaultCityLat: cleanLat, defaultCityLng: cleanLng },
    })
  }
  ...
  const res = NextResponse.json({ ok: true, city: cleanCity, lat: cleanLat, lng: cleanLng })
  res.cookies.set(LOCATION_COOKIE, locationCookieValue(cleanCity, cleanLat, cleanLng), LOCATION_COOKIE_OPTIONS)
  return res
}
