...
**Resolved this session (chat, earlier), remove from any older list:** `GEN-2609-052`, `GEN-2609-053`.

**GEN-2609-054/055/056 dispatched this session (CC-side) — status below, not yet merged:**
- `GEN-2609-056` (spinner-overlay extraction) — **built as scoped**, premise held up. Pushed `feat/gen-2609-056-spinner-overlay-extraction`. Found the 3 sites weren't actually byte-identical (ring size/scrim/accent color all differed) and a latent bug (EventCard/EventRow's spin animation depended on some other page-mounted component defining `@keyframes afa-spin` first) — both fixed. Same wider pattern (12+ other files each duplicate-injecting that same keyframe) flagged, not touched — real follow-up candidate.
- `GEN-2609-055` (Badge migration) — **partially built**, real mix found. Pushed `feat/gen-2609-055-badge-migration`: 3 call sites migrated (organiser/tours, organiser/lineup, admin/diary). 3 more call sites are genuine third pill-shapes (admin/feedback's category/severity badges, admin/bookings' FREE tag, artist/events' compensation pills) — flagged, not forced into either `Badge` variant. Several interactive controls (status buttons, tab switcher, retry button, city select, vouch toggle) correctly left alone — `Badge` has no click handling. No standalone "artist applications" page exists; treated as `artist/events/page.tsx` only.
- `GEN-2609-054` (terracotta → form-submit) — **premise substantially wrong, split**. The "11 occurrences/9 files" count was actually 41/13 just within dashboard/organiser (90+/40+ repo-wide), and it's a real mix (buttons with inconsistent padding, chart bars, selection-pill borders, plain link colors, a status-color ternary — not one shape). Built only the clean, independently-verified sub-part: pushed `feat/gen-2609-054-form-submit-migration` migrating `forgot-password`/`reset-password`'s submit buttons to `form-submit` (their real submit buttons don't even use terracotta — that was a red herring in the dispatch; they matched `-053`'s own "8 occurrences in 4 files" `form-submit` extraction note and just hadn't been switched over yet). **The wider terracotta sweep needs a real decision before anyone builds it** — see docs/design.md's `GEN-2609-054` entry for the split recommendation (a dashboard-CTA button variant's padding/sizing needs deciding; the `DECLINED`-status ternary is a `statusStyle.ts` candidate instead, unrelated to buttons).

All three pushed as separate branches off `qa` at `33cc922`, none touch the same lines as each other. Full reasoning, chrome comparisons, and the flagged-not-forced call sites are in `docs/design.md` (three new dated entries) — this list is pointers, not the analysis.
...
5. Merge/review the three pushed branches (`GEN-2609-054/055/056` — compare URLs in `CC_HANDOFF.md`), then decide with Hitesh on the dashboard-CTA variant and the two other flagged decisions above.
