...
**Resolved, remove from any older list:** `GEN-2609-052` through `-059`/`-060`/`-061`/`-062`/`-063`, all merged into `qa` (`qa` HEAD `3decea2`). `-059`'s hover-treatment half is still genuinely open (see below) — the radius half that merged was only ever half the ticket.

**Two more tickets dispatched this session (CC-side), closing out findings `-063` surfaced — both built, pushed, not yet merged:**
- `GEN-2609-064` (remaining button-shaped terracotta) — **10 of 14 real sites migrated** (re-verified fresh: 14, not the 13 estimated). Two-state buttons (organiser's publish-toggle, artist/events' Apply/Waitlist) reused `-061`'s "Save Lineup" style-override pattern. `verify-phone`'s 2 submit buttons went to `form-submit`, not `primary size="lg"` as suggested — they're shaped like the auth-flow family, not dashboard's CTA family, a real correction not a guess. Fixed 4 more `color: 'white'` bugs for free. **4 sites flagged, not built:** `artist/edit`'s dashed-outline "+ Add tour stop" (no `Button` variant supports a dashed border), and 3 pill-radius (999) CTAs across `DisplayNameNudge`/`PhoneVerifyNudge`/`InstallPrompt` — two share one exact shape, the third is a distinct variant. None fit `sm`/`md`/`lg`'s 6/8/8 radius scheme.
- `GEN-2609-065` (RegisterForm error-color fix) — **shipped.** Re-verified fresh: 11 real lines, not 9 (two separate error banners exist, not one; the QA-mode notice is its own third category). 9 genuinely error-semantic sites routed to `--afa-error`, matching `login.tsx`'s exact established banner pattern. The QA-mode dev-otp notice had a different, real bug (amber background paired with a terracotta border) — fixed by matching the border to amber, not routed to error at all. The 9th real site (a suggestion-chip button) left for `-064`'s remit as instructed — **found it's a 4th instance of the same pill-radius mismatch `-064` already flagged**, plus its own off-brand `rgba(196,90,52,...)` shade distinct from `--afa-terracotta`'s real RGB triple. Not built anywhere yet.
...
- `layout.tsx`/`manifest.ts`'s coupled PWA theme color — still flagged, not touched.
- **New this session:** a real pill-shaped `Button` pattern now has **4 known instances** (`DisplayNameNudge`, `PhoneVerifyNudge`, `InstallPrompt`, `RegisterForm`'s suggestion chip) plus a `PosterShareCard`-adjacent off-brand rgba shade on the last one — a genuine candidate for a new `Button` size/variant, not something to keep re-flagging piecemeal each time it's found; `artist/edit`'s dashed-outline button also still needs a real shape decision.

## Session-start checklist (updated)

1. **`git checkout qa && git fetch origin && git reset --hard origin/qa`** — HEAD should be `3decea2` until this session's 2 branches are merged. This step is now a hard requirement, not a suggestion — see `CC_HANDOFF.md`'s Standing rules section for why.
...
5. Merge `GEN-2609-064` and `-065` (compare URLs in `CC_HANDOFF.md`), then take the flagged decisions above back to Hitesh — the recurring pill-shaped `Button` pattern (4 instances now) is probably the highest-leverage one to resolve first, since it keeps resurfacing piecemeal.
