...
**Resolved, remove from any older list:** `GEN-2609-052` through `-058`, all merged into `qa` (`qa` HEAD `78934c8`).

**Five more tickets dispatched this session (CC-side) — all built, pushed, none merged yet:**
- `GEN-2609-059` (VenueCard corner radius) — **radius half shipped** (0→3px, matches `EventCard`). **Hover-treatment half NOT built — the dispatch's premise was backwards.** Fresh grep found `VenueCard` already has the richer hover (`hover-lift-card`'s translateY+shadow, plus its own amber border/title-color transition) that the ticket describes as something to copy *from* `EventCard`; `EventCard`'s actual hover is just a plain border-color fade, nothing else. Matches `GEN-2609-053`'s own prior audit, which documented this gap in the opposite direction. Needs Hitesh to confirm real intent before any hover CSS changes.
- `GEN-2609-060` (Badge sizes) — **fully shipped**. 3 new additive variants (`micro`/`tag`/`pill`), each matched exactly to its real call site(s). Checked the dispatch's own question on the "FREE" tag — it's a fixed fact, not a status, so it got its own `tag` variant rather than being forced under `status-*`.
- `GEN-2609-061` (3 remaining terracotta buttons) — **fully shipped**. "Start Camera Scan"/"Send to all" migrated cleanly (the latter's `#fff` bug fixed for free); "Save Lineup" needed the `style`-override escape hatch since its background is gated on `dirty` independent of the button's own `disabled` state — no new `Button` prop added for one caller.
- `GEN-2609-062` (consolidate `@keyframes afa-spin`) — **fully shipped**. Moved into `globals.css`, all 13 real duplicate injections across 10 files removed. Found `dashboard/artist/page.tsx`'s spinner used `--afa-terracotta` (every other one uses `--afa-fill-solid`) — flagged for `-063`, which then fixed it.
- `GEN-2609-063` (repo-wide non-button terracotta → fill-solid) — **fully shipped, and it uncovered a real gap.** Actual count was ~90 occurrences across ~40 files, not the ~27 estimated. All genuine non-button usages migrated in 6 logical commits (DECLINED ternary → `statusStyle.ts`'s new `orange` tone; charts; selection pills incl. 2 more found via literal-RGB grep; everything else). **Two things flagged, not built:** (1) **13 real button-shaped terracotta sites exist outside `dashboard/organiser/`** that `-058`/`-061` never covered (`dashboard/artist/`, `dashboard/venue/`, `verify-phone`, `AudienceChoiceVoting`, `DisplayNameNudge`, `PhoneVerifyNudge`, `InstallPrompt`, `PosterShareCard`) — 4 of them carry the same `color: 'white'` bug fixed twice before. Strong candidate for a `-058`-shaped follow-up ticket. (2) `RegisterForm.tsx`'s 9 terracotta usages are flagged as a unit — 8 are error/validation-state colors, and the same file already uses `--afa-error` for the identical semantic concept elsewhere, so blindly swapping to fill-solid would just replace one wrong color-role with another; needs a real decision (probably route to `--afa-error` instead), not a token rename. `layout.tsx`'s `themeColor` also flagged — it's coupled to `manifest.ts`'s hardcoded `theme_color` hex, so swapping only one side would break a documented invariant.

**One real cross-branch overlap found before pushing, not silently combined:** `dashboard/artist/page.tsx` is touched by both `-062` (removes a duplicate keyframe `<style>` tag) and `-063` (changes the same spinner's color, 3 lines away) — compatible changes, but close enough in the same JSX block that a clean auto-merge isn't guaranteed. Flag this at merge time rather than assume it resolves itself.
...
- `PhoneVerifyNudge.tsx` legacy tokens (`BUG-2609-029`, still open at last check) — note this file is also one of `-063`'s 13 flagged button sites now.
...
- **New this session:** `GEN-2609-059`'s hover-treatment direction needs Hitesh's confirmation; the 13 remaining button-shaped terracotta sites outside `dashboard/organiser/` (real follow-up ticket candidate); `RegisterForm.tsx`'s error-color-vs-fill-solid decision; `layout.tsx`/`manifest.ts`'s coupled PWA theme color.

## Session-start checklist (updated)

1. **`git checkout qa && git fetch origin && git reset --hard origin/qa`** — HEAD should be `78934c8` until this session's 5 branches are merged. This step is now a hard requirement, not a suggestion — see `CC_HANDOFF.md`'s Standing rules section for why.
...
5. Merge `GEN-2609-059` through `-063` (compare URLs in `CC_HANDOFF.md`; watch for the `dashboard/artist/page.tsx` overlap between `-062`/`-063` at merge time), then take the 4 flagged decisions above back to Hitesh.
